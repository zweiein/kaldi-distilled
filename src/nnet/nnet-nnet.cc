...
void Nnet::SetDropoutRetention(BaseFloat r)  {
  //KALDI_ERR << "SetDropoutRetention " << r  ;
  for (int32 c=0; c < NumComponents(); c++) {
    if (GetComponent(c).GetType() == Component::kDropout) {
      ...
      KALDI_ERR << "Setting dropout-retention in component " << c 
                << " from " << r_old << " to " << r 
                << "Component= " << GetComponent(c).GetType()
                << " kActivationFunction" << Component::kActivationFunction;
    }
  }
}
